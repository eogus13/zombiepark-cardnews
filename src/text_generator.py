...
2. 각 장별 이미지 생성용 영문 프롬프트 1개씩
   (스타일: cinematic survival horror, dark moody, neon green accents,
    Instagram 1:1, high contrast, dark purple background tones)
...
def _fallback_text(content: dict) -> dict:
    ...
    for slide in content.get("slides", []):
        slides.append({
            ...
            "text": slide.get("text", ""),
            "image_prompt": (
                f"Cinematic survival horror scene, dark moody atmosphere, "
                f"neon green accents, abandoned island, "
                f"Instagram square 1:1, high contrast, "
                f"dark purple background, {content.get('type', 'zombie')}"
            )
        })
